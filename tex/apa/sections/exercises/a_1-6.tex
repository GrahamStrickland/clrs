...
\begin{proof}
  ...
  \begin{equation*}
    \begin{split}
      \sum_{k = 1}^n O(f_k(i)) = \, &O(f_1(i)) + O(f_2(i)) + \cdots +
      O(f_n(i)) \\
  ...
  Interpreting summation in the above as set union, we have a
  superset $\psi$ where
  $O(f_1(i)) \subseteq \psi, O(f_2(i)) \subseteq \psi, \ldots
  O(f_n(i)) \subseteq \psi$,
  since $\psi = \sum_{k = 1}^n O(f_k(i))$.

  Now, by the linearity property, $f_1(i) + f_2(i) + \cdots f_n(i) =
  \sum_{k = 1}^n f_k(i)$
